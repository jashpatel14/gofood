...
class _FoodDetailsScreenState extends ConsumerState<FoodDetailsScreen> {
  ...
  double _calculateTotalPrice(FoodModel food, int quantity) {
    if (_addons == null) return 0;
    double addonTotal = 0;
  ...
  Widget build(BuildContext context) {
    ...
    return foodAsync.when(
      ...
      data: (foodData) {
        ...
        final currentQty = isInCart ? cartState.items[cartItemIndex].quantity : _quantity;
        final totalPrice = _calculateTotalPrice(food, currentQty);
                ...
                children: [
                  ...
                  Expanded(
                    child: CustomButton(
                      ...
                      icon: isClosedOrPaused ? Icons.block : (isInCart ? Icons.shopping_cart : Icons.shopping_bag_outlined),
                      onPressed: isClosedOrPaused 
                          ? null 
                          : () {
                              if (isInCart) {
                                context.push('/cart');
                      ...
                      Row(
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(
                              border: Border.all(color: food.isVeg ? AppColors.success : AppColors.error, width: 2),
